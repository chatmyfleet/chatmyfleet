...
- **Five-minute install holds.** Someone who has never touched ChatMyFleet before clones the public repo, runs `docker compose up` on a fresh Linux box, and gets a webhook-connected agent replying in Slack within five minutes. First try. No manual config diving.
...
- **One protocol-layer connector at launch: the generic HTTP/webhook connector.** (Originally the brief launched on an "OpenAI Assistants" connector — **dropped 2026-05-28**: OpenAI does not host a server-invocable agent — the Assistants API sunsets 2026-08-26 and Agent Builder is ChatKit/browser-only. See `docs/spikes/spike-2-openai-poll.md`.) The generic webhook connector is promoted to the v0.4 launch slot; the remaining protocol-layer connectors (which carry no affiliate commission) shift up one — **MCP at v0.5.0** (~2026-09), **LangGraph remote-graph at v0.10.0** (~2027-02). MCP at v0.5 already covers LangGraph users who speak MCP; the standalone LangGraph connector is for those who don't. The Wave 1 affiliate-partner integrations (locked order below) ride on this generic webhook connector.
...
2. ~~**OpenAI Assistants `requires_action` POLL pattern**~~ — **CLOSED** (OpenAI dropped as a connector target). Replaced by connection spikes for the Wave 1 platforms (n8n / Dify / Lindy / Flowise) over the generic webhook connector — see `docs/spec.md` §5 Spikes 10–13.
