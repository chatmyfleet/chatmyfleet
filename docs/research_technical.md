...
- **Connector:** Generic HTTP/webhook connector (CMF→agent POST + agent→CMF reply/`approval_request` endpoints) — the v0.4 launch connector after OpenAI Assistants was dropped (Spike 2 Closed). Wave 1 platforms (n8n, Dify, Lindy, Flowise) ride on it
...
| **Generic HTTP/webhook connector** | `httpx` (outbound) + FastAPI (inbound endpoint) | **The v0.4 LAUNCH connector** — promoted from v0.5 after OpenAI Assistants was dropped (Spike 2 Closed: OpenAI hosts no server-invocable agent). CMF POSTs the operator message to the agent's webhook; the agent replies via CMF's `POST /v1/agents/{id}/messages` (or a sync response); HITL via the `approval_request` endpoint. Covers n8n / Lindy webhooks + Dify / Flowise REST + any custom HTTP agent | Bespoke per-platform connectors (more code; the Wave 1 platforms ride on this webhook instead) |
| MCP connector | `mcp` Python SDK | Deferred to **v0.5.0** (shifted up from v0.6 after OpenAI removal) — not in v0.4 dependency tree | — |
| LINE adapter | `line-bot-sdk` | Deferred to **v0.8.0** — not in v0.4 dependency tree | — |
| LangGraph connector | `langgraph-sdk` | Deferred to **v0.10.0** (shifted up from v0.11) — not in v0.4 dependency tree | — |
...
| 2 | ~~OpenAI Assistants POLL pattern~~ → **Spike 2 CLOSED**: OpenAI dropped as a connector target (no server-invocable hosted agent). Connector risk now lives in the Wave 1 platform connection spikes | Spikes **10–13** — connect to n8n / Dify / Lindy / Flowise over the generic webhook + HITL callback | Epic 5 (Generic HTTP/Webhook Connector) | per-platform accounts |
...
| Wave 1 platform API change (n8n/Dify/Lindy/Flowise) between Loop 1 and launch | Generic webhook connector isolates per-platform specifics behind a small adapter; integration tests against recorded fixtures per platform; manual SIT in `tests/manual/epic-X.md` runs against the live platform before each tag |
...
1. **Phase 1 — Foundation (Week 1-3, no public tag).** Skeleton: 4 containers (`cmf-gateway`, `cmf-worker`, `postgres`, `redis`), CI green, Postgres migrations for 8 tables (`tenants`, `users`, `agents`, `conversations`, `approval_requests`, `audit_events`, `memory_rules`, `memory_entities`, `memory_decisions`), gateway + worker boot, Slack adapter (signature verify + Block Kit send), **generic HTTP/webhook connector** (CMF→agent POST + agent→CMF reply), conversation state machine (5 states), audit log table, basic slash commands. **Spikes 1, 7, 8, 9 resolved here** (8 → Epic 1, 7 → Epic 2, 9 → Epic 3, 1 → Epic 4). (Spike 2 closed — OpenAI dropped; Wave 1 platform connection spikes 10–13 validate the webhook connector against real platforms when accounts exist.)
2. **Phase 2 — v0.1.0 PUBLIC REPO (Week 4, Wed Jun 24).** Trust-signal artifacts populated (LICENSE, TRADEMARK, SECURITY, CONTRIBUTING, MAINTENANCE, CHANGELOG, AFFILIATES). `chatmyfleet.com` minimal landing live. Sponsors page live. **Repo flips public.** First moment a self-hoster can clone, run `docker compose up`, and have their webhook-connected agent talking to Slack with an audit trail.
...
6. **Phase 6 — Post-launch monthly cadence (Sep 2026 → Apr 2027+).** The generic webhook connector is now the v0.4 launch connector (not post-launch). Protocol connectors shifted up one after OpenAI was dropped: **v0.5 MCP** (was v0.6), **v0.10 LangGraph** (was v0.11). Affiliate + channel targets unchanged: v0.7 n8n, v0.8 LINE + Dify, v0.9 Lindy + audit Ed25519 signing + hash chain, v0.10 Flowise + Telegram, v0.12 Helm + Wave 2 affiliate. The Wave 1 affiliate integrations (n8n/Dify/Lindy/Flowise) are marketplace nodes riding on the launch webhook connector. v1.0.0 ships when the API surface is locked and the product is fully production-stable (expected ~M12 anniversary or later).
