...
| Embedding provider | **Default candidate pending Spike 3** — `intfloat/multilingual-e5-small` (MIT, 384-dim, ~470MB) via FastEmbed; alternative `google/embeddinggemma-300m` (Apache 2.0 + Gemma terms, 768-dim default with Matryoshka to 128/256/512, ~1.2GB). Operator-swappable at install time via `EMBEDDING_MODEL` env var | Local-default keeps the OSS promise intact (zero paid API keys required for memory features to work); FastEmbed avoids `torch` in the container (~100MB ONNX runtime vs ~500MB+ torch); multilingual coverage including Thai is required for user-zero and the v0.8 LINE adapter | OpenAI `text-embedding-3-small` (1536-dim, BYOK — works but locks adopters to OpenAI for memory); Voyage / Cohere Embed (BYOK, extra provider); local `sentence-transformers` + torch (heavyweight container); Jina v3 (CC BY-NC, blocked for commercial OSS) |
